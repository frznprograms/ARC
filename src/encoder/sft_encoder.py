...
class ReviewClassificationDataset(Dataset):
    def __init__(
        self, prompts: List[str], labels: List[Dict], tokenizer, max_length: int = 512
    ):
...
class ReviewSFTEncoder(nn.Module):
    ...
    def predict_review_classification(
        self, review_prompt: str, threshold: float = 0.5
...
class ReviewDataProcessor:
    ...
    def load_labeled_data_from_csv(self, csv_path: str) -> tuple[List[str], List[Dict]]:
        ...
        for _, row in df.iterrows():
            ...
            label_dict = {
                ...
                "irrelevant_content": int(row.get("relevance", 0)),
    ...
    def create_prompt_response_pairs(
        self, df: pd.DataFrame, labeled_data: List[Dict]
...
class ReviewSFTTrainer:
    ...
    def evaluate(self, dataloader: DataLoader) -> Dict[str, float]:
        ...
        metrics["overall_accuracy"] = np.mean(
            [metrics.get(f"{cat}_accuracy", 0) for cat in categories]
        )
...
def main():
    ...
    # Create datasets
    train_dataset = ReviewClassificationDataset(
        train_prompts, train_labels, model.tokenizer
    )
