...
- Reviews that clearly violate platform guidelines""",
    ...
    description_examples = "; ".join(
        [desc for desc in generator.descriptions[:5] if desc is not None]
    )
        ...
        try:
            response = generator.client.chat.completions.create(
                ...
                messages=[
                    ...
                    {
                        "role": "user",
                        "content": f"Generate {current_batch_size} {classification_type} reviews in the specified JSON format.",
                    },
                ],
            ...
            for json_str in json_matches:
                try:
                    ...
                    review_data["label_spam"] = (
                        1 if classification_type == "spam" else 0
                    )
                    review_data["label_advertisement"] = (
                        1 if classification_type == "advertisement" else 0
                    )
                    review_data["label_irrelevant_content"] = (
                        1 if classification_type == "irrelevant_content" else 0
                    )
                    review_data["label_non_visitor_rant"] = (
                        1 if classification_type == "non_visitor_rant" else 0
                    )
                    review_data["label_toxicity"] = (
                        1 if classification_type == "toxicity" else 0
                    )
            ...
            print(
                f"  Generated batch {batch_start // batch_size + 1}, total so far: {len(all_reviews)}"
            )
            ...
            fallback_review = {
                **business_info,
                ...
                "label_advertisement": 1
                if classification_type == "advertisement"
                else 0,
                "label_irrelevant_content": 1
                if classification_type == "irrelevant_content"
                else 0,
                "label_non_visitor_rant": 1
                if classification_type == "non_visitor_rant"
                else 0,
...
def check_current_status():
    ...
    files_to_check = {
        "advertisement": [
            "synthetic_reviews_advertisement_7200.csv",
            "synthetic_reviews_advertisement_3000.csv",
        ],
        "irrelevant_content": [
            "synthetic_reviews_irrelevant_content_7200.csv",
            "synthetic_reviews_irrelevant_content_3000.csv",
        ],
...
def generate_with_active_saving(
    generator: SyntheticReviewGenerator,
    class_type: str,
    num_reviews: int,
    batch_size: int = 20,
    ...
    for filename in possible_files:
        if os.path.exists(filename):
            try:
                ...
                print(
                    f"  Loaded {len(existing_reviews)} existing reviews from {filename}"
                )
                break
            except Exception as e:
    ...
    print(
        f"  Target: {target_total} total reviews ({len(existing_reviews)} existing + {num_reviews} new)"
    )
    ...
    for batch_num in range(batches_needed):
        ...
        print(
            f"  Batch {batch_num + 1}/{batches_needed}: Generating {current_batch_size} reviews..."
        )
        ...
        try:
            batch_reviews = generate_reviews_with_llm(
                generator, class_type, current_batch_size, batch_size=current_batch_size
            )
            all_reviews.extend(batch_reviews)
            new_reviews_generated += len(batch_reviews)

            print(
                f"  Generated {len(batch_reviews)} reviews (Total: {len(all_reviews)}, New: {new_reviews_generated})"
            )
...
def clean_and_combine_datasets():
    ...
    label_cols = [
        "label_spam",
        "label_advertisement",
        "label_irrelevant_content",
        "label_non_visitor_rant",
        "label_toxicity",
    ]
    ...
    if "user_message" not in final_df.columns:
        ...
        final_df["user_message"] = (
            "Business Name: "
            + final_df["business_name"].astype(str).fillna("N/A")
            + "\n"
            + "Category: "
            + final_df["category"].astype(str).fillna("N/A")
            + "\n"
            + "Description: "
            + final_df["description"].astype(str).fillna("N/A")
            + "\n"
            + "Review: "
            + final_df["review"].astype(str).fillna("N/A")
            + "\n"
            + "Rating: "
            + final_df["rating"].astype(str)
            + "\n"
            + "Response: "
            + final_df["response"].astype(str).fillna("N/A")
        )
...
def main():
    ...
    print(f"\n🔍 FINAL VERIFICATION")
    fallback_check = (
        final_dataset["text"].str.contains("Fallback", case=False, na=False).sum()
    )
    ...
    print(
        f"Reviews with exactly 1 label: {single_label_count:,}/{len(final_dataset):,}"
    )
    print(
        f"Validation status: {'PASSED' if single_label_count == len(final_dataset) else 'FAILED'}"
    )
