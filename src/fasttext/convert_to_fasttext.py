...
def main():
    ap = argparse.ArgumentParser(description="Convert labeled reviews to fastText files.")
    ap.add_argument("input_path", help="Path to JSON array or JSONL file of samples")
    ap.add_argument(
        "-o", "--out-dir", default="fasttext_out",
    ...
    print(f"Done. Wrote: {', '.join(cat + '.txt' for cat in CATEGORIES)} in {args.out_dir}")
